package CoffeeRecords::Web::Plugin::Session;
use strict;
use warnings;
use utf8;
...
my $cipher = Crypt::CBC->new({
    key => 'AECQRZFcxd0fYq3aLUOj07304tO4FyGn',
    cipher => 'Rijndael',
});
sub _session {
    my $self = shift;
    ...
    if (!exists $self->{session}) {
        $self->{session} = HTTP::Session2::ClientStore2->new(
            env => $self->req->env,
            secret => 'qLmia6PpfQQCvIoyaWR87CCvJk74gAVX',
            cipher => $cipher,
        );
    }
    return $self->{session};
...
This module manages session for CoffeeRecords.
